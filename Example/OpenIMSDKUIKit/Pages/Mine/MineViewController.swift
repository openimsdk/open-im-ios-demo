...
public class MineViewController: UIViewController {
    private lazy var headerView: HeaderView = {
        let v = HeaderView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: 222.0 / 375.0 * UIScreen.main.bounds.width + 15))
        ...
        v.tap.rx.event.subscribe(onNext: { [weak self] _ in
            ...
            let vc = QRCodeViewController(idString: IMController.addFriendPrefix + user.userID)
    ...
    private let items: [RowType] = [.myInfo, .setting, .aboutUs, .logout]
...
extension MineViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if item == .aboutUs {
            ...
            cell.titleLabel.text = item.title + "app:\(majorVersion) / build:\(minorVersion)"
        }
        return cell
    }
