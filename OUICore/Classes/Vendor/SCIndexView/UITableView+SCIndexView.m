...
- (void)SCIndexView_layoutSubviews {
    ...
    CGRect rect = CGRectMake(self.frame.origin.x, self.frame.origin.y + self.sc_indexViewConfiguration.indicatorCenterYOffset, self.frame.size.width, self.frame.size.height);
    
    if (!CGRectEqualToRect(self.sc_indexView.frame, rect)) {
        self.sc_indexView.frame = rect;
    }
