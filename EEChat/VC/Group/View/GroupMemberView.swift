...
class GroupMemberView: UIView {
    ...
    var didSelectUser: ((UserInfo) -> Void)?
...
//                })
            OpenIMiOSSDK.shared().getGroupMembersInfo(groupID!, uidList: [uid]) { members in
                DispatchQueue.main.async {
                    if let member = members.first {
                        if (member as AnyObject).role == 0 || (member as AnyObject).role == 1 {
                            SelectGroupMemberVC.show(op: .removeMember, groupID: groupID!)
                        }else{
                            MessageModule.showMessage(NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "You're not an administrator."]))
                        }
                    }else{
