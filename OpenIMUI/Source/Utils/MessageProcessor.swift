...
public final class MessageProcessor: NSObject {
    ...
    private override init() {
        self.path = MessageProcessor.cachePath
        super.init()
    ...
    public func image(_ image: UIImage, url: URL?) -> Message {
        var image = image
        let path: String
        if let url = url {
        ...
        return OpenIMiOSSDK.shared().createImageMessage(fromFullPath: path)
    }
    
    public func audio(_ url: URL) throws -> Message {
        assert(url.isFileURL)
        ...
        try? FileManager.default.copyItem(atPath: url.relativePath, toPath: path)
//        return OpenIMiOSSDK.shared().createSoundMessage(path, duration: Int(ceil(duration)))
        return OpenIMiOSSDK.shared().createSoundMessage(fromFullPath: path, duration: Int(ceil(duration)))
    }
    
    public func video(_ url: URL, callback: @escaping (Message?) -> Void ) {
        assert(url.isFileURL)
...
//                let message = OpenIMiOSSDK.shared().createVideoMessage(url.relativePath, videoType: "mp4", duration: Int(ceil(duration)), snapshotPath: snapshotPath)
                let message = OpenIMiOSSDK.shared().createVideoMessage(fromFullPath: url.relativePath, videoType: "mp4", duration: Int(ceil(duration)), snapshotPath: snapshotPath)
    ...
    public func file(_ url: URL) -> Message {
        let path = self.path + "file/" + url.lastPathComponent
        let newURL = URL(fileURLWithPath: path)
        try? FileManager.default.removeItem(at: newURL)
        try? FileManager.default.copyItem(at: url, to: newURL)
//        return OpenIMiOSSDK.shared().createFileMessage(path, fileName: url.lastPathComponent)
        return OpenIMiOSSDK.shared().createFileMessage(fromFullPath: path, fileName: url.lastPathComponent)
    }
