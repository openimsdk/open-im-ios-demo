...
open class IMConversationViewController: UIViewController,
    ...
    public init(conversation: ConversationInfo) {
...
//            OIMManager.addAdvancedMsgListener(self)
            OpenIMiOSSDK.shared().addAdvancedMsgListenerWith { msgId in
                
            } onRecvC2CReadReceipt: { haveReadInfo in
                
            } onRecvNewMessage: { Message in
...
//        OIMManager.markMessageHasRead(uid: conversation.userID, gid: conversation.groupID)
        OpenIMiOSSDK.shared().markGroupMessageHasRead(conversation.groupID ?? "") { msg in
...
//            OIMManager.setConversationDraft(conversation.conversationID, draftText: text)
            OpenIMiOSSDK.shared().setConversationDraft(conversation.conversationID ?? "", draft: text) { msg in
...
//        }
        ...
        OpenIMiOSSDK.shared().getHistoryMessageList(conversation.userID!, groupID: conversation.groupID!, startMsg: nil, count: count, onSuccess: { [weak self] array in
            var msgs: [MessageType] = []
            for d in array {
                msgs.append(MessageType(message: d))
            }
            DispatchQueue.main.async {
                let collectionView = self!.messageVC.collectionView
                let isEmpty = self!.messages.isEmpty
                self!.messages.insert(contentsOf: self!.preprocess(messages: msgs), at: 0)
                if isEmpty {
                    collectionView.reloadData()
                    collectionView.scrollToLastItem(at: .bottom, animated: false)
                } else {
    ...
    public func sendMessage(_ message: MessageType) {
        ...
        message.status = .none
        OpenIMiOSSDK.shared().send(message.innerMessage, recvUid: conversation.userID!, recvGid: conversation.groupID!, onlineUserOnly: false) { msg in
            DispatchQueue.main.async {
                MessageProcessor.shared.removeProgress(msgID: message.messageId, isFinish: true)
                message.status = .success
            }
        } onProgress: { p in
            MessageProcessor.shared.hookProgress(msgID: message.messageId)(p)
        } onError: { code, msg in
            DispatchQueue.main.async {
...
//        OpenIMiOSSDK.shared().sendMessage(message.innerMessage,
...
//        }
    ...
    open func onRecvNewMessage(_ message: Message) {
...
//        let message = OpenIMiOSSDK.shared().create(text, at: uids).toUIMessage()
//        let message = OpenIMiOSSDK.shared().createText(atMessage: text, atUidList: uids).toUIMessage()
        let message = OpenIMiOSSDK.shared().createTextMessage(text).toUIMessage();
...
//            OpenIMiOSSDK.shared().getGroupMembersInfo(conversation.groupID,
//                                           uids: [message.userID])
//            { result in
...
//                }
//            }
            OpenIMiOSSDK.shared().getGroupMembersInfo(conversation.groupID!, uidList: [message.userID]) { result in
                DispatchQueue.main.async {
                    if let member = result.first {
                        avatarView.setImage(with: (member as AnyObject).faceUrl, placeholder: avatarView.image)
                    }
                }
            } onError: { code, msg in
