...
class ChatTableViewCell: UITableViewCell {
    ...
    let unreadLabel: RoundCornerLayoutLabel = {
        let v = RoundCornerLayoutLabel.init(roundCorners: .allCorners, radius: nil)
        v.font = .systemFont(ofSize: 12)
        v.backgroundColor = StandardUI.color_F44038
        v.textColor = .white
        v.textAlignment = .center
        v.contentInset = UIEdgeInsets.init(top: 1, left: 4, bottom: 1, right: 4)
        return v
    }()
    
    let muteImageView: UIImageView = {
        let v = UIImageView.init(image: UIImage.init(nameInBundle: "chat_status_muted_icon"))
        v.isHidden = true
        return v
    }()
