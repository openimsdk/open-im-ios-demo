...
class SearchRecordViewModel {
    ...
    func searchText(_ text: String?) {
        ...
        let param = SearchParam.init()
        param.conversationID = self.conversationId
    ...
    private func divideMessagesToSection(messages: [MessageInfo]) -> [MediaSectionModel] {
        ...
        for message in messages {
            ...
            var sendDate: Date = Date.init(timeIntervalSince1970: sendTime)
        ...
        for sectionName in map.keys.sorted() {
            ...
            sections.append(MediaSectionModel.init(model: sectionName, items: values))
        }
        return sections
    }
}
