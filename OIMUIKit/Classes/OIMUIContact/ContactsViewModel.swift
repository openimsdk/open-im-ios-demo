...
public class ContactsViewModel {
    ...
    let companyDepartments: [Department] = [
        Department.init(isHost: true, name: "托云信息技术有限公司"),
        Department.init(isHost: false, name: "组织架构"),
        Department.init(isHost: false, name: "技术部")
    ]
    ...
    init() {
        ...
        IMController.shared.friendInfoChangedSubject.subscribe(onNext: { [weak self] (userInfo: UserInfo?) in
            ...
            if let user = oldValues.first(where: {return $0.userID == userInfo.userID}) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.applicationCountChangedHandler), name: ContactsViewModel.NotificationApplicationCountChanged, object: nil)
    }
    ...
    static let NotificationApplicationCountChanged = NSNotification.Name.init(rawValue: "OIMNotificationApplicationCountChanged")
