...
class GroupListResultViewController: UIViewController, UISearchResultsUpdating, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let group = dataList.first(where: { $0.groupID == person.personId }) {
            cell.avatarImageView.setImage(with: group.faceURL, placeHolder: "contact_my_friend_icon")
        }
        return cell
    }
