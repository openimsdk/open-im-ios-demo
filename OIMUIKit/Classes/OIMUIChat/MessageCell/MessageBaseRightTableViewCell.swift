...
class MessageBaseRightTableViewCell: UITableViewCell, MessageCellAble {
    ...
    lazy var bubbleImageView: UIImageView = {
        let v = UIImageView()
        v.isUserInteractionEnabled = true
        v.image = UIImage.init(nameInBundle: "msg_bubble_right_image")?.resizableImage(withCapInsets: UIEdgeInsets.init(top: 20, left: 5, bottom: 5, right: 15))
    ...
    lazy var resendBtn: UIButton = {
        let v = UIButton()
        v.setImage(UIImage.init(nameInBundle: "msg_send_fail_icon"), for: .normal)
    ...
    lazy var messageStack: UIStackView = {
        ...
        let v = UIStackView.init(arrangedSubviews: [nameLabel, bubbleImageView])
    ...
    lazy var stateStack: UIStackView = {
        let v = UIStackView.init(arrangedSubviews: [resendBtn, indicatorView, readLabel])
        v.axis = .horizontal
        v.distribution = .equalSpacing
        return v
    }()
