...
class MessageListViewModel {
    ...
    func markAudio(messageId: String, isPlaying: Bool) {
        var origin = self.messagesRelay.value
        var changed: Bool = false
        for (index, item) in self.messagesRelay.value.enumerated() {
        ...
        if changed {
            self.messagesRelay.accept(origin)
        }
    }
    ...
    private func sortMessages(messages: [MessageInfo]) -> [MessageInfo] {
        let ret = messages.sorted { lh, rh in
            return lh.sendTime > rh.sendTime
        }
        return ret
    }
}
