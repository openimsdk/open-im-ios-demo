...
class ChatRecordView: UIView {
    ...
    let recordBtn: UIButton = {
        let v = UIButton()
        v.isSelected = true
        v.setBackgroundImage(UIImage.init(nameInBundle: "inputbar_record_background_selected"), for: .selected)
        v.setBackgroundImage(UIImage.init(nameInBundle: "inputbar_record_background_unselected"), for: .normal)
        return v
    }()
    
    private lazy var recordingImageView: AnimationView = {
        let bundle = ViewControllerFactory.getBundle() ?? Bundle.main
        let v = AnimationView.init(name: "voice_record", bundle: bundle)
        v.loopMode = .autoReverse
        return v
    }()
    
    private lazy var recordingBackgroundImageView: UIImageView = {
        let v = UIImageView.init(image: UIImage.init(nameInBundle: "chattool_voice_background_white_image")?.resizableImage(withCapInsets: UIEdgeInsets.init(top: 8, left: 8, bottom: 10, right: 8), resizingMode: UIImage.ResizingMode.stretch))
        v.addSubview(recordingImageView)
        recordingImageView.snp.makeConstraints { make in
            make.size.equalTo(CGSize.init(width: 140, height: 35))
    ...
    private lazy var cancelRecordingBackgroundImageView: UIImageView = {
        let v = UIImageView.init(image: UIImage.init(nameInBundle: "chattool_voice_background_red_image")?.resizableImage(withCapInsets: UIEdgeInsets.init(top: 8, left: 8, bottom: 10, right: 8), resizingMode: UIImage.ResizingMode.stretch))
        v.addSubview(cancelRecordingImageView)
        cancelRecordingImageView.snp.makeConstraints { make in
            make.size.equalTo(CGSize.init(width: 41, height: 10))
            make.center.equalToSuperview()
        }
        v.isHidden = true
        return v
    ...
    lazy var cancelBtn: UIButton = {
        let v = UIButton()
        v.setImage(UIImage.init(nameInBundle: "inputbar_record_cancel_btn_icon_selected"), for: .selected)
        v.setImage(UIImage.init(nameInBundle: "inputbar_record_cancel_btn_icon_unselected"), for: .normal)
    ...
    lazy var convertBtn: UIButton = {
        let v = UIButton()
        v.setImage(UIImage.init(nameInBundle: "inputbar_record_convert_btn_icon_selected"), for: .selected)
        v.setImage(UIImage.init(nameInBundle: "inputbar_record_convert_btn_icon_unselected"), for: .normal)
        return v
    }()

    override init(frame: CGRect) {
        super.init(frame: frame)
        let recordIcon = UIImageView.init(image: UIImage.init(nameInBundle: "inputbar_record_voice_icon"))
        ...
        self.addSubview(recordingBackgroundImageView)
        recordingBackgroundImageView.snp.makeConstraints { make in
            make.center.equalToSuperview()
            make.size.equalTo(CGSize.init(width: 194, height: 95))
        }
        
        self.addSubview(cancelRecordingBackgroundImageView)
        cancelRecordingBackgroundImageView.snp.makeConstraints { make in
            make.centerX.equalTo(cancelBtn)
            make.size.equalTo(CGSize.init(width: 104, height: 95))
            make.centerY.equalToSuperview()
        }
        
        self.addSubview(convertRecordingImageView)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            ...
            _cancelBtnSelectedObservable.onNext(self.cancelBtn.frame.contains(point))
            _convertBtnSelectedObservable.onNext(self.convertBtn.frame.contains(point))
        }
    }
