...
class FileHelper {
    ...
    func getContentTypeOf(imageData: NSData) -> String? {
        ...
        switch (c) {
            ...
            let string: NSString = NSString.init(data: imageData.subdata(with: NSRange.init(location: 0, length: 12)), encoding: String.Encoding.ascii.rawValue) ?? ""
    ...
    func isGif(imageData: NSData) -> Bool {
        if self.getContentTypeOf(imageData: imageData) == "gif" {
            return true
        }
        return false
    }
